...
//  * Calls the n8n webhook (no session ID), resets the session, clears the chat, 
//  * and shows the new inactivity message.
//  * (MODIFIED to be async and call webhook with no data)
//  */
// async function handleInactivity() {
//     console.log(`User inactive for 1 minutes. Resetting session: ${sessionIdLeft}`);

//     // (NEW) Call the inactivity webhook with NO session ID
//     try {
//         console.log('Calling inactivity webhook...');
        
//         await fetch(N8N_INACTIVITY_WEBHOOK_URL, {
//             method: 'GET',
//             mode: 'cors'
//         });
...
//     sessionIdLeft = 'IMT_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9);
//     localStorage.setItem('chatSessionIdIMT', sessionIdLeft);
//     console.log(`New session ID created: ${sessionIdLeft}`);

//     // 2. Clear the chat history from the UI
//     chatMessagesLeft.innerHTML = ''; 

//     // 3. (NEW) Add the inactivity message
...
function cleanN8NResponse(rawText) {
    ...
    for (const line of lines) {
        ...
        // Check if the line starts with '{' or '[' (basic JSON check)
        if (trimmedLine.startsWith('{') || trimmedLine.startsWith('[')) {
            // This line looks like JSON, so we stop processing
            // and don't include it or any subsequent lines.
            break;
        }
        
        // This line is not JSON, so add it to our clean response
        cleanLines.push(line);
    }

    // Join the clean lines back together and trim any trailing/leading whitespace
...
// const N8N_WEBHOOK_URL_LEFT = 'https://zoximasolutionss.app.n8n.cloud/webhook/700de22e-bc3d-4ff2-9086-0b9b0e347f85/chat';
const N8N_WEBHOOK_URL_LEFT = 'https://zoximasolutionss.app.n8n.cloud/webhook/faf0cc3f-df67-4ca4-a016-75e7e1b34827/chat';
// Webhook URL for inactivity
const N8N_INACTIVITY_WEBHOOK_URL = 'https://zoximasolutionss.app.n8n.cloud/webhook/2b722ab5-f463-4607-a6dc-addc0b3cbd9f';
